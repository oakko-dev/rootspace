...
test("makes cached data available without waiting for a delayed refresh", () => {
  const storage = new MemoryStorage();
...
test("does not expose one user's cache to another user", () => {
  ...
  assert.equal(parseBookmarkBoardCache(storage.getItem(getBookmarkBoardCacheKey("user-1")), "user-2"), null);
});
