...
function isBadRequest(error) {
	return error instanceof BookmarkBoardError && error.status === 400;
}
...
test("validates discriminated bookmark board actions", () => {
	...
	assert.throws(() => parseBookmarkBoardAction({ action: "unknown" }), isBadRequest);
});
...
test("loads collections and bookmarks in parallel-shaped queries", async () => {
	...
	assert.match(result.bookmarks[0].faviconUrl, /favicon\.ico$/u);
...
test("delete collection removes it and persists remaining positions", async () => {
	...
	assert.deepEqual(supabase.calls.find((call) => call.operation === "delete").filters, [
		["user_id", "user-1"],
		["id", "old"],
	]);
	assert.equal(
