...
export function getBookmarkHost(url) {
	try {
		return new URL(normalizeBookmarkUrl(url)).hostname.replace(/^www\./iu, "");
	} catch {
		return "";
	}
}
...
function decodeHtmlEntity(entity) {
	...
	if (value.startsWith("#")) {
		return String.fromCodePoint(Math.trunc(Number(value.slice(1))));
	}
...
export function extractHtmlTitle(html) {
	const titleMatch = String(html || "").match(/<title[^>]*>(?<title>[\s\S]*?)<\/title>/iu);
	if (!titleMatch) {
		return "";
	}
	...
	return titleMatch.groups.title
		.replaceAll(/<[^>]+>/gu, "")
		.replaceAll(/&(?:amp|apos|gt|lt|quot|#\d+|#x[\da-f]+);/giu, decodeHtmlEntity)
		.replaceAll(/\s+/gu, " ")
		.trim();
}
...
function getHtmlAttribute(tag, attribute) {
	const match = String(tag).match(
		new RegExp(
			`\\b${attribute}\\s*=\\s*(?:"(?<double>[^"]*)"|'(?<single>[^']*)'|(?<bare>[^\\s>]+))`,
			"iu",
		),
	);
	return (match?.groups?.double || match?.groups?.single || match?.groups?.bare || "").replaceAll(
		/&amp;/giu,
		"&",
	);
}
...
export function extractHtmlFaviconUrl(html, pageUrl) {
	const linkTags = String(html || "").match(/<link\b[^>]*>/giu) || [];

	for (const tag of linkTags) {
		const rel = getHtmlAttribute(tag, "rel");
		if (!/(?:^|\s)(?:icon|shortcut|apple-touch-icon)(?:\s|$)/iu.test(rel)) {
			continue;
		}
		...
		try {
			...
			if (["http:", "https:"].includes(faviconUrl.protocol)) {
...
export function filterBookmarkBoard(collections, bookmarks, query) {
	...
	return sortedCollections
		...
		.filter(
			(collection) =>
				collection.bookmarks.length > 0 || textMatches(collection.name, normalizedQuery),
		);
}
...
export function parseBookmarkBoardExport(value) {
	...
	if (!Array.isArray(parsed.collections) || !Array.isArray(parsed.bookmarks)) {
		throw new TypeError("Import must include collections and bookmarks arrays.");
	}
