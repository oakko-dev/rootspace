...
test("normalizes URLs and creates host title fallbacks", () => {
  ...
  assert.equal(getBookmarkFaviconUrl("https://www.example.com/docs"), "https://www.example.com/favicon.ico");
});
...
test("rejects invalid or unsupported URLs", () => {
  assert.throws(() => normalizeBookmarkUrl("ftp://example.com"), /Only http and https/);
  assert.throws(() => normalizeBookmarkUrl("not a url"), /valid URL/);
});
...
test("filters board by collection and bookmark text", () => {
  ...
  const bookmarks = [
    { id: "crm", collectionId: "work", title: "CRM", url: "https://crm.example.com", description: "" },
    { id: "paper", collectionId: "read", title: "Paper", url: "https://papers.example.com", description: "AI notes" },
  ];
  ...
  assert.deepEqual(
    filterBookmarkBoard(collections, bookmarks, "ai").map((collection) => collection.bookmarks.map((bookmark) => bookmark.id)),
    [["paper"]],
  );
});

test("exports and parses board JSON shape", () => {
  const collections = [{ id: "inbox", name: "Inbox", description: "", color: "#7dd3fc", position: 0 }];
  const bookmarks = [
    {
      id: "openai",
      collectionId: "inbox",
