...
const bookmarkBoardActionSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("upsert-collections"), collections: z.array(collectionSchema).max(500) }),
  z.object({
...
export async function resolveBookmarkBoardContext(getUser, createClient) {
  ...
  if (!user) throw new BookmarkBoardError("Authentication required.", 401);
...
export async function loadBookmarkBoard(supabase, userId, createId = () => crypto.randomUUID()) {
  ...
  return {
    ...
    missingFaviconBookmarks: rawBookmarks.filter((bookmark) => !bookmark.favicon_url).map(bookmarkFromDb),
  };
}
