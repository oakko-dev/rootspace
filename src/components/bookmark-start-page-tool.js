...
const showBookmarkAlert = (message) => Reflect.get(globalThis, "alert")(message);
const confirmBookmarkAction = (message) => Reflect.get(globalThis, "confirm")(message);
...
class BookmarkBoardRequestError extends Error {
	constructor(message, status) {
		super(message);
		this.name = "BookmarkBoardRequestError";
		this.status = status;
	}
}
...
async function requestBookmarkBoard(url, options) {
	const response = await fetch(url ?? "/api/bookmark-board", { cache: "no-store", ...options });
...
function moveByOffset(list, id, offset) {
	...
	if (index === -1 || targetIndex === -1 || targetIndex >= list.length) {
		return list;
	}
	const next = [...list];
...
export default function BookmarkStartPageTool() {
	...
	const [bookmarkOpeningPreference, setBookmarkOpeningPreference] =
		useState(BOOKMARK_OPENING_NEW_TAB);
	...
	const [bookmarkDropPreview, setBookmarkDropPreview] = useState({
		collectionId: "",
		bookmarkId: "",
	});
	...
	async function upsertCollections(nextCollections) {
		if (!canEdit || nextCollections.length === 0) {
			return;
		}
	...
	async function upsertBookmarks(nextBookmarks) {
		if (!canEdit || nextBookmarks.length === 0) {
			return;
		}
		await postBookmarkBoardAction({
	...
	async function mutateBoard(operation, nextBoard) {
		if (!canEdit) {
			return;
		}
		...
		try {
			...
			if (mutationRevision.current === revision) {
				if (nextBoard) {
					writeBookmarkBoardCache(user.id, nextBoard);
				}
				setSyncState("saved");
				setTimeout(() => {
					if (mutationRevision.current === revision) {
			...
			showBookmarkAlert(error instanceof Error ? error.message : "Failed to sync bookmark board.");
		}
	}
	...
	useEffect(() => {
		...
		async function loadBoard(hintedUserId = "") {
			...
			try {
				...
				if (!freshBoard) {
					throw new Error("Bookmark board response was invalid.");
				}
				...
				if (disposed || boardRequestRevision.current !== requestRevision) {
					return;
				}

				if (error instanceof BookmarkBoardRequestError && error.status === 401) {
					if (hintedUserId) {
						clearBookmarkBoardCache(hintedUserId);
					}
					clearBookmarkBoardUser();
	...
	useEffect(() => {
		...
		function syncCachedBoard(event) {
			if (event.key !== getBookmarkBoardCacheKey(user.id) || !event.newValue) {
				return;
			}
	...
	function runContextMenuAction(operation) {
		setContextMenu((current) => ({ ...current, open: false }));
		operation();
	}
	...
	async function saveCollection(event) {
		...
		closeDialogs();
		await mutateBoard(() => upsertCollections([nextCollection]), {
			collections: nextCollections,
			bookmarks,
		});
	}
	...
	async function deleteCollection(collection) {
		...
		if (collections.length <= 1) {
			showBookmarkAlert("Keep at least one collection on the board.");
			return;
		}
		if (!confirmBookmarkAction(`Delete "${collection.name}" and its bookmarks?`)) {
			return;
		}
	...
	async function saveBookmark(event) {
		...
		let { faviconUrl } = bookmarkForm;

		try {
			({ url: normalizedUrl } = { url: normalizeBookmarkUrl(bookmarkForm.url) });
			const urlChanged =
				!bookmarkDialog.item || normalizeBookmarkUrl(bookmarkDialog.item.url) !== normalizedUrl;
			if (urlChanged || !title || !faviconUrl) {
				const metadata = await fetchBookmarkMetadata(normalizedUrl);
				const { title: metadataTitle, faviconUrl: metadataFaviconUrl } = metadata;
				title ||= metadataTitle;
				faviconUrl = metadataFaviconUrl;
			}
		} catch (error) {
		...
		const nextBookmark = {
			...
			position:
				bookmarkDialog.item?.position ??
				bookmarks.filter((item) => item.collectionId === bookmarkForm.collectionId).length,
		...
		setBookmarks(nextBookmarks);
		closeDialogs();
		await mutateBoard(() => upsertBookmarks([nextBookmark]), {
			collections,
			bookmarks: nextBookmarks,
		});
	}
	...
	async function deleteBookmark(bookmark) {
		...
		if (!confirmBookmarkAction(`Delete "${bookmark.title}"?`)) {
			return;
		}
	...
	async function reorderCollections(sourceId, targetId) {
		...
		setCollections(nextCollections);
		await mutateBoard(() => upsertCollections(nextCollections), {
			collections: nextCollections,
			bookmarks,
		});
	}
	...
	async function reorderBookmark(sourceBookmark, targetCollectionId, targetBookmarkId = "") {
		...
		if (sourceBookmark.id === targetBookmarkId) {
			return;
		}

		const remainingBookmarks = bookmarks.filter((bookmark) => bookmark.id !== sourceBookmark.id);
		const collectionBookmarks = remainingBookmarks.filter(
			(bookmark) => bookmark.collectionId === targetCollectionId,
		);
		...
		const nextBookmarks = [
			...
			if (reorderedIds.has(bookmark.id)) {
				return bookmark;
			}
			const siblings = remainingBookmarks.filter(
				(item) => item.collectionId === bookmark.collectionId,
			);
			return { ...bookmark, position: siblings.findIndex((item) => item.id === bookmark.id) };
		});

		setBookmarks(nextBookmarks);
		await mutateBoard(() => upsertBookmarks(nextBookmarks), {
			collections,
			bookmarks: nextBookmarks,
		});
	}
	...
	async function importPastedUrls(event) {
		...
		try {
			...
			const currentCount = bookmarks.filter(
				(bookmark) => bookmark.collectionId === targetCollectionId,
			).length;
			...
			setBookmarks(nextBookmarks);
			closeDialogs();
			await mutateBoard(() => upsertBookmarks(importedBookmarks), {
				collections,
				bookmarks: nextBookmarks,
			});
		} catch (error) {
	...
	if (dbError) {
		return <Alert variant="destructive">{dbError}</Alert>;
	}
	...
	return (
		<div className="min-h-full overflow-hidden text-foreground">
										...
										<div className="flex items-start gap-3 px-7">
											<div className="min-w-0">
												<div className="flex items-center gap-3">
													<span
														className="h-2.5 w-2.5 rounded-full opacity-90"
														style={{ backgroundColor: collection.color }}
													/>
													<h2 className="truncate text-xl font-extrabold tracking-normal text-[#e9e9ec]">
														{collection.name}
													</h2>
												</div>
												{collection.description ? (
													<p className="mt-2 line-clamp-2 text-xs leading-5 text-[#777985]">
														{collection.description}
													</p>
												) : null}
											</div>
											...
											{collection.bookmarks.map((bookmark) => (
												<div key={bookmark.id}>
													<div
														...
														draggable={canEdit}
														onContextMenu={(event) =>
															openContextMenu(event, "bookmark", { collection, bookmark })
														}
														onDragStart={(event) => {
															event.stopPropagation();
															event.dataTransfer.effectAllowed = "move";
													...
													>
														...
														{bookmark.description ? (
															<p className="mt-1 line-clamp-2 text-xs leading-5 text-[#777985]">
																{bookmark.description}
															</p>
														) : null}
													</div>
				...
				>
							...
							<button
								...
								onClick={() =>
									runContextMenuAction(() =>
										window.open(contextMenu.bookmark.url, "_blank", "noreferrer"),
									)
								}
								role="menuitem"
							>
					...
					{contextMenu.type === "collection" && contextMenu.collection ? (
						<>
							<button
								...
								className="block w-full px-3 py-2 text-left hover:bg-[#282930] disabled:cursor-not-allowed disabled:opacity-40"
								onClick={() =>
									runContextMenuAction(() => openBookmarkDialog(null, contextMenu.collection.id))
								}
								disabled={!canEdit}
								role="menuitem"
							>
							...
							<button
								...
								className="block w-full px-3 py-2 text-left hover:bg-[#282930] disabled:cursor-not-allowed disabled:opacity-40"
								onClick={() =>
									runContextMenuAction(() => openPasteImportDialog(contextMenu.collection.id))
								}
								disabled={!canEdit}
								role="menuitem"
							>
							...
							<hr className="my-1 border-[#2a2b31]" />
							<button
								type="button"
								className="block w-full px-3 py-2 text-left hover:bg-[#282930] disabled:cursor-not-allowed disabled:opacity-40"
								onClick={() =>
									runContextMenuAction(() => openCollectionDialog(contextMenu.collection))
								}
								disabled={!canEdit}
								role="menuitem"
							>
							...
							<button
								...
								className="block w-full px-3 py-2 text-left hover:bg-[#282930] disabled:cursor-not-allowed disabled:opacity-40"
								onClick={() =>
									runContextMenuAction(() => moveCollection(contextMenu.collection.id, -1))
								}
								disabled={
									!canEdit ||
									collections.findIndex((item) => item.id === contextMenu.collection.id) === 0
								}
								role="menuitem"
							>
								Move left
							...
							<button
								...
								disabled={
									!canEdit ||
									collections.findIndex((item) => item.id === contextMenu.collection.id) ===
					...
					{contextMenu.type === "board" ? (
						<>
							...
							<hr className="my-1 border-t border-[#2a2b31]" />
							<button
								type="button"
								className="block w-full px-3 py-2 text-left hover:bg-[#282930] disabled:cursor-not-allowed disabled:opacity-40"
			...
			<Dialog
				...
				onOpenChange={(open) =>
					setCollectionDialog({ open, item: open ? collectionDialog.item : null })
				}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{collectionDialog.item ? "Edit collection" : "Add collection"}
						</DialogTitle>
					</DialogHeader>
								...
								{COLORS.map((color) => (
									<button
										...
										className={cn(
											...
											collectionForm.color === color && "ring-2 ring-ring",
										)}
			...
			<Dialog
				...
				onOpenChange={(open) =>
					setBookmarkDialog({
						open,
						item: open ? bookmarkDialog.item : null,
						collectionId: bookmarkDialog.collectionId,
					})
				}
			>
				<DialogContent>
					...
					<form className="space-y-4" onSubmit={saveBookmark}>
							...
							<Input
								...
								placeholder={
									titleLookupState === "loading"
										? "Fetching page title..."
										: "Uses page title or host if empty"
								}
							/>
							{titleLookupState === "loading" ? (
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={(collectionId) => setBookmarkForm({ ...bookmarkForm, collectionId })}
							>
						...
						<div className="space-y-2">
							...
							<Textarea
								...
								onChange={(event) =>
									setBookmarkForm({ ...bookmarkForm, description: event.target.value })
								}
							/>
						</div>
						<Button type="submit" className="w-full" disabled={!canEdit}>
			...
			<Dialog
				...
				onOpenChange={(open) => setImportDialog({ open, mode: importDialog.mode })}
			>
				<DialogContent className="max-w-2xl">
					<DialogHeader>
						<DialogTitle>
							{importDialog.mode === "json" ? "Import board JSON" : "Paste URLs"}
						</DialogTitle>
					</DialogHeader>
					<form
						className="space-y-4"
						onSubmit={importDialog.mode === "json" ? importJson : importPastedUrls}
					>
						{formError ? <Alert variant="destructive">{formError}</Alert> : null}
						{importDialog.mode === "paste" ? (
							<div className="space-y-2">
								...
								<Select
									value={bookmarkForm.collectionId || collections[0]?.id}
									onValueChange={(collectionId) =>
										setBookmarkForm({ ...bookmarkForm, collectionId })
									}
								>
						...
						<div className="space-y-2">
							<Label htmlFor="import-text">
								{importDialog.mode === "json" ? "JSON" : "One URL per line"}
							</Label>
							<Textarea
								id="import-text"
