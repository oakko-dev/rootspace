...
const showPlannerAlert = (message) => Reflect.get(globalThis, "alert")(message);
const confirmPlannerAction = (message) => Reflect.get(globalThis, "confirm")(message);
const requestPlannerInput = (message) => Reflect.get(globalThis, "prompt")(message);
...
function getInstallmentRemainingBalance(item) {
...
async function loadPlannerData(supabase, userId) {
	const [
		...
		supabase
			.from(PLANNER_TABLES.income)
			.select("id,planner_year,name,notes,monthly")
			.eq("user_id", userId),
		supabase
			.from(PLANNER_TABLES.deductions)
			.select("id,planner_year,name,notes,monthly")
			.eq("user_id", userId),
		supabase
			.from(PLANNER_TABLES.expenses)
			.select("id,planner_year,name,notes,amount,expense_type,icon")
			.eq("user_id", userId),
		supabase
			.from(PLANNER_TABLES.installments)
			.select("id,planner_year,card_id,name,notes,monthly,start_month,end_month")
			.eq("user_id", userId),
		supabase
			.from(PLANNER_TABLES.incomeActuals)
			.select("income_id,planner_year,planner_month,amount")
			.eq("user_id", userId),
		supabase
			.from(PLANNER_TABLES.deductionActuals)
			.select("deduction_id,planner_year,planner_month,amount")
			.eq("user_id", userId),
	]);
...
async function deleteUserRows(supabase, tableName, userId) {
	const result = await supabase.from(tableName).delete().eq("user_id", userId);
	throwIfSupabaseError(result.error);
}

async function insertRows(supabase, tableName, rows) {
	if (rows.length > 0) {
		const result = await supabase.from(tableName).insert(rows);
		throwIfSupabaseError(result.error);
	}
}
...
export default function FinancialPlannerTool() {
	...
	useEffect(() => {
		async function loadFinancialData() {
			try {
				...
				setDbError(
					message === "table-not-found" ? "table-not-found" : `Database error: ${message}`,
				);
			} finally {
				setLoading(false);
			}
	...
	const saveToSupabase = async (newData) => {
		...
		try {
			...
			showPlannerAlert(
				`Failed to sync data to Supabase: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	};
	...
	const handleDeleteItem = (type, id) => {
		if (!confirmPlannerAction("Are you sure you want to delete this item?")) {
			return;
		}
		...
		if (type === "income") {
			yearData.income = yearData.income.filter((item) => item.id !== id);
		}
		if (type === "deduction") {
			yearData.deductions = yearData.deductions.filter((item) => item.id !== id);
		}
		if (type === "expense") {
			yearData.expenses = yearData.expenses.filter((item) => item.id !== id);
		}
		if (type === "installment") {
			yearData.installments = yearData.installments.filter((item) => item.id !== id);
		}
	...
	const totalInstallments = yearData.installments.reduce(
		(acc, item) => acc + Number(item.monthly || 0),
		0,
	);
	...
	return (
		<div className="space-y-8">
			...
			<div className="flex flex-wrap items-center justify-between gap-3">
				...
				<Badge
					variant={savingState === "error" ? "destructive" : savingState ? "success" : "secondary"}
				>
					{savingState || "idle"}
				</Badge>
			</div>
						...
						<Card key={card.id}>
							<CardHeader className="flex-row items-center justify-between">
								...
								<Badge variant="secondary">
									{formatCurrency(getCardInstallmentTotal(card.name))}
								</Badge>
							</CardHeader>
							<CardContent className="space-y-2">
								{yearData.installments
									.filter((i) => i.card === card.name)
									.map((item) => {
											...
											>
												<div>
													...
													<p className="text-xs text-muted-foreground">
														...
														{formatCurrency(getInstallmentRemainingBalance(item))} remaining
													</p>
												</div>
												<div className="flex gap-2">
													<Button
														...
														variant="ghost"
														size="icon"
														onClick={() => handleOpenEditModal("installment", item)}
													>
														<Icons.Edit3 size={16} />
													</Button>
													<Button
														...
														onClick={() => handleDeleteItem("installment", item.id)}
													>
					...
					<Card>
						...
						<CardContent>
								...
								{data.cards.map((card) => (
									...
									>
										...
										<Button
											...
											className="hover:text-red-300"
											onClick={() => {
												if (confirmPlannerAction("Are you sure?")) {
													const updated = {
														...data,
							...
							<Button
								...
								className="mt-4"
								onClick={() => {
									const name = requestPlannerInput("Enter card name:");
									if (name) {
										const updated = {
											...data,
											cards: [...data.cards, { id: createClientId(), name, color: "#65d9f2" }],
										};
										saveToSupabase(updated);
									}
								}}
